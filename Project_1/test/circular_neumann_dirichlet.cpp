...
int main() {
    // 读取 JSON 文件
    std::ifstream config_file("../json/circular_neumann_dirichlet.json");
    ...
    for (const auto& test_case : config["test_cases"]) {
        ...
        domain.setSquareBoundaryCondition(BCType::Neumann, boundary_funcs);

        // 设置圆形边界条件
        std::string circle_func_name = test_case["circle_boundary"]["function"];
        domain.setCircleBoundaryCondition(BCType::Dirichlet, function_map[circle_func_name]);
